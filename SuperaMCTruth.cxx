...
namespace larcv {
  ...
  SuperaMCTruth::SuperaMCTruth(const std::string name) : SuperaBase(name) {}
  ...
  bool SuperaMCTruth::process(IOManager& mgr)
  {
    ...
    for (auto const& label : _producer_labels) {
      ...
      for (size_t mct_index = 0; mct_index < mct_v.size(); ++mct_index) {
        ...
        if (_pass_origin && mct.Origin() != _pass_origin) continue;
        ...
        if (mct.NeutrinoSet()) {
          ...
          nu.momentum(
            mcnu.Momentum(0).X() * 1.e3, mcnu.Momentum(0).Y() * 1.e3, mcnu.Momentum(0).Z() * 1.e3);
          nu.position(
            mcnu.Position(0).X(), mcnu.Position(0).Y(), mcnu.Position(0).Z(), mcnu.Position(0).T());
          ...
          for (size_t j = 0; j < traj.size(); ++j) {
            neutrino.add_trajectory_point(traj.X(j),
                                          traj.Y(j),
                                          traj.Z(j),
                                          traj.T(j),
                                          traj.Px(j),
        ...
        for (int i = 0; i < mct.NParticles(); ++i) {
          ...
          if (mcp.StatusCode() != 1)
            std::cout << "bad particle: track id = " << mcp.TrackId() << " PDG = " << mcp.PdgCode()
          ...
          p.shape((mcp.PdgCode() == 11 || mcp.PdgCode() == 22 ? larcv::kShapeShower :
                                                                larcv::kShapeTrack));
