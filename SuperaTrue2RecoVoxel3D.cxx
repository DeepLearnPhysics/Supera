  ...
  for (auto const& sim_ch : sim_chnls) {
    ...
    for (auto const& tick_ides : sim_ch.TDCIDEMap()) {
      ...
      for (auto const& edep : tick_ides.second) {
        out << ch << ',' << time << ',' << edep.trackID << ',' << edep.numElectrons << ','
            << edep.energy << ',' << edep.x << ',' << edep.y << ',' << edep.z << ','
...
inline void dump_reco_hits(T const& hits, std::string fname)
{
  ...
  for (auto const& hit : hits) {
    out << hit.Channel() << "," << hit.PeakTime() << "," << hit.RMS() << "," << hit.PeakAmplitude()
        << "," << hit.Integral() << "\n";
  }
  out.close();
}
...
inline void dump_cluster3d(T const& space_pts, F const& finder, M const& meta, std::string fname)
{
  std::ofstream out(fname);
  ...
  for (size_t i = 0; i < space_pts.size(); ++i) {
    ...
    out << meta.id(xyz[0], xyz[1], xyz[2]) << ',' << xyz[0] << ',' << xyz[1] << ',' << xyz[2] << ','
        << pt.ErrXYZ()[1];
    ...
    for (auto const& hit : hits) {
      out << ',' << hit->Channel() << ',' << hit->PeakTime() << ',' << hit->RMS();
    }
    out << "\n";
  }
  out.close();
...
inline void dump_ghosts(T const& ghosts, M const& meta, std::string fname)
{
  ...
  for (const auto& [vox_id, is_ghost] : ghosts) {
    out << vox_id << ',' << meta.pos_x(vox_id) << ',' << meta.pos_y(vox_id) << ','
        << meta.pos_z(vox_id) << ',' << is_ghost << '\n';
  }
  out.close();
}
...
inline void dump_reco2true(T const& reco2true, std::string fname)
{
  ...
  for (auto const& [reco_pt, true_hits] : reco2true) {
    for (auto const& hit : true_hits) {
      out << reco_pt.get_id() << ',' << hit.voxel_id << ',' << hit.track_id << '\n';
    }
  }
  out.close();
}
...
inline void dump_true2reco(T const& true2reco, std::string fname)
{
  ...
  for (auto& [true_info, reco_pts] : true2reco) {
    for (auto reco_pt : reco_pts) {
      ...
      out << true_info.voxel_id << ',' << true_info.track_id << ',' << reco_id << '\n';
    }
  }
  out.close();
}
...
namespace larcv {
  ...
  SuperaTrue2RecoVoxel3D::SuperaTrue2RecoVoxel3D(const std::string name) : SuperaBase(name) {}
  ...
  void SuperaTrue2RecoVoxel3D::configure(const PSet& cfg)
  {
    ...
    if (_sps_producer_v.size() == 0) LARCV_ERROR() << "No space point producers" << std::endl;
    ...
    if (_dump_to_csv) {
      ...
      for (size_t ch = 0; ch < geom->Nchannels(); ++ch) {
        ...
        for (auto const& id : ids) {
          out << ch << ',' << id.Cryostat << ',' << id.TPC << ',' << id.Plane << ',' << id.Wire
              << '\n';
        }
      }
      out.close();
  ...
  bool SuperaTrue2RecoVoxel3D::process(IOManager& mgr)
  {
    ...
    for (auto const& sch : LArData<supera::LArSimCh_t>()) {
      ...
      for (auto const& tick_ides : sch.TDCIDEMap()) {
        double x_pos = (supera::TPCTDC2Tick(tick_ides.first) * supera::TPCTickPeriod() -
                        supera::TriggerOffsetTPC()) *
                       supera::DriftVelocity();
        TrueHit_t hit;
        ...
        for (auto const& edep : tick_ides.second) {
          ...
          hit.track_voxel_ids.emplace_back(vox_id,
                                           _useOrigTrackID ? edep.origTrackID : edep.trackID);
          hit.n_electrons.push_back(edep.numElectrons);
        }

        // append hit to true_hit_vv[ch]
        if (hit.track_voxel_ids.size()) true_hit_vv[ch].push_back(std::move(hit));
      }
    }
    //meta3d.update(meta3d.num_voxel_x()*_voxel_size_factor, meta3d.num_voxel_y()*_voxel_size_factor, meta3d.num_voxel_z() * _voxel_size_factor);

    LARCV_INFO() << "Created a list of true hits: " << true_hit_vv.size() << " channels"
                 << std::endl;
    if (_debug) {
      size_t num_hits = 0;
      for (auto const& true_hit_v : true_hit_vv)
        num_hits += true_hit_v.size();
    ...
    for (auto _sps_producer : _sps_producer_v) {
      ...
      for (size_t i = 0; i < space_pts->size(); ++i) {
        ...
        if (_twofold_matching) {
          ...
          for (size_t plane = 1; plane < matched_voxels.size(); ++plane) {
            ...
            if (_voxel_distance_threshold < 0) {
              std::set_intersection(overlaps.begin(),
                                    overlaps.end(),
                                    v.begin(),
                                    v.end(),
      ...
      LARCV_DEBUG() << "Dropping " << n_dropped << " out of " << space_pts->size()
    ...
    if (_dump_to_csv) dump_true2reco(_true2reco, save_to("true2reco_all"));
    ...
    LARCV_INFO() << true2reco.size() << " true points mapped to " << reco2true.size()
                 << " reco points" << std::endl;
    ...
    if (!_output_tensor3d.empty() || !_output_cluster3d.empty()) {
      ...
      if (!_output_cluster3d.empty()) {
        event_cluster3d =
          (larcv::EventClusterVoxel3D*)(mgr.get_data("cluster3d", _output_cluster3d));
      ...
      for (auto const& keyval : true2reco) {
        ...
        for (auto const& reco_id : keyval.second)
          vs.emplace(reco_id, 0., true);
        ++cluster_ctr;
      }
    }
    ...
    if (gaus_hits.isValid()) { dump_reco_hits(*gaus_hits, save_to("gaushit")); }
  ...
  void SuperaTrue2RecoVoxel3D::set_intersection_distance(const larcv::Voxel3DMeta& meta3d,
                                                         ...
                                                         std::set<TrackVoxel_t>& overlaps)
  {
    ...
    auto comp = [&](const TrackVoxel_t& lhs, const TrackVoxel_t& rhs) {
      ...
      double distance = std::max(std::max(std::fabs(lpos.x - rpos.x) / meta3d.size_voxel_x(),
                                          std::fabs(lpos.y - rpos.y) / meta3d.size_voxel_y()),
                                 std::fabs(lpos.z - rpos.z) / meta3d.size_voxel_z());
  ...
  void SuperaTrue2RecoVoxel3D::set_intersection_factor(const larcv::Voxel3DMeta& meta3d,
                                                       const std::set<TrackVoxel_t>& v1,
                                                       const std::set<TrackVoxel_t>& v2,
                                                       std::set<TrackVoxel_t>& overlaps)
  {
    ...
    if (_voxel_size_factor == 1.) {
      std::set_intersection(
        v1.begin(), v1.end(), v2.begin(), v2.end(), std::inserter(overlaps, overlaps.end()));
      return;
    }
    // else we start by creating an alternate meta
    // with large voxels
    auto meta_translated = meta3d;
    meta_translated.update(meta3d.num_voxel_x() / _voxel_size_factor,
                           meta3d.num_voxel_y() / _voxel_size_factor,
                           meta3d.num_voxel_z() / _voxel_size_factor);
    ...
    std::set_intersection(v1_translated.begin(),
                          v1_translated.end(),
                          v2_translated.begin(),
                          v2_translated.end(),
    ...
    std::set_union(v1.begin(), v1.end(), v2.begin(), v2.end(), std::inserter(all_v, all_v.end()));
  ...
  void SuperaTrue2RecoVoxel3D::set_ghost_with_averaging(const larcv::Voxel3DMeta& meta3d)
  {
                                         ...
                                         std::set<TrackVoxel_t>& track_voxel_ids)
  {
    for (auto const& hit : hits) {
      ...
      if (t_start <= hit.time && hit.time <= t_end)
        track_voxel_ids.insert(hit.track_voxel_ids.begin(), hit.track_voxel_ids.end());
    }
  }
  ...
  const std::unordered_set<TrackVoxel_t>& SuperaTrue2RecoVoxel3D::find_true(VoxelID_t reco_id) const
  {
  ...
  const std::unordered_set<RecoVoxel3D>& SuperaTrue2RecoVoxel3D::find_reco(int track_id,
                                                                           VoxelID_t true_id) const
  {
  ...
  std::map<VoxelID_t, std::unordered_set<VoxelID_t>> SuperaTrue2RecoVoxel3D::contract_true2reco()
  {
  ...
  std::map<VoxelID_t, std::unordered_set<VoxelID_t>> SuperaTrue2RecoVoxel3D::contract_reco2true()
  {
