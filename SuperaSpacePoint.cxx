...
namespace larcv {
  ...
  SuperaSpacePoint::SuperaSpacePoint(const std::string name) : SuperaBase(name) {}
  void SuperaSpacePoint::configure(const PSet& cfg)
  {
    ...
    if (_producer_labels.size() == 0) { LARCV_ERROR() << "No SpacePointProducer(s)\n"; }
  ...
  bool SuperaSpacePoint::process(IOManager& mgr)
  {
    ...
    for (auto const& label : _producer_labels) {
      ...
      for (size_t i_pt = 0; i_pt < points.size(); ++i_pt) {
        if (i_pt < 10 || (i_pt < 1000 && i_pt % 100 == 0) || (i_pt < 10000 && i_pt % 1000 == 0) ||
            (i_pt % 10000 == 0)) {
          LARCV_INFO() << "Processing " << label << ": " << i_pt << "/" << points.size() << '\n';
        }
        ...
        if (charge < _reco_charge_range[0] || charge > _reco_charge_range[1]) { continue; }
        ...
        if (_store_wire_info) {
          if (hits.size() > _n_planes) {
            LARCV_WARNING()
              ...
              << "Wrong number of hits: " << hits.size() << " (expecting " << _n_planes << ")"
              << std::endl;
            ++n_dropped;
            continue;
          }
      ...
      LARCV_INFO() << n_dropped << " out of " << points.size() << " SpacePoints dropped from "
                   << label << std::endl;
    }
  ...
  float SuperaSpacePoint::get_common_charge(const std::vector<art::Ptr<recob::Hit>>& hits)
  {
    ...
    if (charge_total <= 0 || idx0 >= idx1) return 0;
