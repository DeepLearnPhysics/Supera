...
namespace larcv {
  ...
  SuperaWire::SuperaWire(const std::string name) : SuperaBase(name) {}
  ...
  void SuperaWire::configure(const PSet& cfg)
  {
    ...
    for (auto const& cryo_id : cryostat_v) {
      ...
      if (_scan.size() <= cryo_id) _scan.resize(cryo_id + 1);
      for (auto const& tpc_id : tpc_v) {
        if (!cryostat.HasTPC(tpc_id)) continue;
        auto const& tpc = cryostat.TPC(tpc_id);
        if (_scan[cryo_id].size() <= tpc_id) _scan[cryo_id].resize(tpc_id + 1);
                                                   ...
                                                   const double x_max)
  {
    ...
    double min_time =
      (dist_min / supera::DriftVelocity() - supera::TriggerOffsetTPC()) / supera::TPCTickPeriod();
    double max_time =
      (dist_max / supera::DriftVelocity() - supera::TriggerOffsetTPC()) / supera::TPCTickPeriod();
    ...
    if ((mid_time + num_time_half) > supera::NumberTimeSamples()) {
      ...
      LARCV_INFO() << "mid_time + num_time_half (" << mid_time + num_time_half << ") abvoe max ("
                   << supera::NumberTimeSamples() << ")"
                                                   ...
                                                   const geo::Point_t& max_pt)
  {
    ...
    */
    ...
    if (wid0 < 0) { wid0 = 0; }
    if (wid0 >= (int)(plane_geo.Nwires())) { wid0 = plane_geo.Nwires() - 1; }
    if (wid1 < 0) { wid1 = 0; }
    if (wid1 >= (int)(plane_geo.Nwires())) { wid1 = plane_geo.Nwires() - 1; }
    if (wid2 < 0) { wid2 = 0; }
    if (wid2 >= (int)(plane_geo.Nwires())) { wid2 = plane_geo.Nwires() - 1; }
    if (wid3 < 0) { wid3 = 0; }
    if (wid3 >= (int)(plane_geo.Nwires())) { wid3 = plane_geo.Nwires() - 1; }
    ...
    if ((mid_wire + num_wire_half) > plane_geo.Nwires()) {
      ...
      LARCV_INFO() << "mid_wire + num_wire_half (" << mid_wire + num_wire_half << ") abvoe max ("
                   << plane_geo.Nwires() << ")"
  ...
  bool SuperaWire::process(IOManager& mgr)
  {
    ...
    for (unsigned int cryo_id = 0; cryo_id < _scan.size(); ++cryo_id) {
      ...
      for (unsigned int tpc_id = 0; tpc_id < tpcs.size(); ++tpc_id) {
        ...
        for (unsigned int plane_id = 0; plane_id < planes.size(); ++plane_id) {
          ...
          LARCV_INFO() << "Creating meta: " << cryo_id << "-" << tpc_id << "_" << plane_id
                       << std::endl;
          // get the meta
          ...
          meta2d = ImageMeta(wrange.first,
                             trange.first,
                             wrange.second + 1,
                             trange.second + 1,
                             _npx_rows,
                             _npx_columns,
                             cryo_id * 100 + tpc_id * 10 + plane_id,
                             larcv::kUnitCM);
    ...
    for (auto const& wire : wire_v) {
      ...
      if (idx >= (int)(meta2d_v.size())) {
        LARCV_WARNING() << idx << "Unexpected: " << meta2d_v.size() << " " << vs_v.size()
                        << std::endl;
        throw larbys();
      }
