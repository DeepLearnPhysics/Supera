...
namespace supera {
  ...
  void MCParticleTree::DefineNeutrinoPrimary(const std::vector<supera::LArMCTruth_t>& mctruth_v)
  {
	...
	root_node.part.EnergyDeposit(0);
	root_node.part.EnergyInit(0);
	root_node.part.Position(vtx);
	root_node.part.PdgCode(mcp.PdgCode());
	root_node.part.Momentum(mom.X()*1.e3, mom.Y()*1.e3, mom.Z()*1.e3);
	root_node.part.EnergyInit(mom.E() * 1.e3);
	root_node.part.MCTIndex(mctruth_idx);
	if(mctruth.NeutrinoSet()) {
	  root_node.part.NuCurrentType(mctruth.GetNeutrino().CCNC());
	  root_node.part.NuInteractionType(mctruth.GetNeutrino().InteractionType());
	}
