...
namespace supera {
  ...
  WTRangeArray_t MCParticleHelper::WireTimeBoundary(const LArMCTrack_t& mct) const
  {
    LARCV_DEBUG() << "start" << std::endl;
  ...
  WTRangeArray_t MCParticleHelper::WireTimeBoundary(const LArMCShower_t& mcs) const
  {
    LARCV_DEBUG() << "start" << std::endl;
    const double drift_velocity = ::supera::DriftVelocity() * 1.0e-3; // make it cm/ns
    //const int tick_max = ::supera::NumberTimeSamples();
  ...
  WTRangeArray_t MCParticleHelper::WireTimeBoundary(const LArMCShower_t& mcs, const std::vector<supera::LArSimCh_t>& sch_v) const
  {
    LARCV_DEBUG() << "start" << std::endl;
    // result is N planes' wire boundary + time boundary (N+1 elements)
    WTRangeArray_t result(::supera::Nplanes() + 1);
  ...
  std::vector<larcv::BBox2D> MCParticleHelper::WTRange2BB(const WTRangeArray_t& wtrange_v) const
  {
    LARCV_DEBUG() << "start" << std::endl;
	...
	origin_x = (int)(wrange.Start()) - (int)(_wire_padding);
      }
	...
	origin_y = (int)(trange.Start()) - (int)(_time_padding);
      ...
      bb_v.emplace_back(origin_x,origin_y,origin_x+width,origin_y+height);
    }
    return bb_v;
  }

  std::vector<larcv::BBox2D> MCParticleHelper::MakeBBox2D( const supera::LArMCTrack_t& mct, const int time_offset) const
  {
    LARCV_DEBUG() << "start" << std::endl;
  ...
  std::vector<larcv::BBox2D> MCParticleHelper::MakeBBox2D( const supera::LArMCTrack_t& mct, const std::vector<supera::LArSimCh_t>& sch_v, const int time_offset) const
  {
    LARCV_DEBUG() << "start" << std::endl;
  ...
  std::vector<larcv::BBox2D> MCParticleHelper::MakeBBox2D( const supera::LArMCShower_t& mcs, const int time_offset) const
  {
    LARCV_DEBUG() << "start" << std::endl;
  ...
  std::vector<larcv::BBox2D> MCParticleHelper::MakeBBox2D( const supera::LArMCShower_t& mcs, const std::vector<supera::LArSimCh_t>& sch_v, const int time_offset) const
  {
    LARCV_DEBUG() << "start" << std::endl;
